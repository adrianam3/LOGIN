...
export class TicketListComponent {
        ...
        try {
            ...
            if (data && Array.isArray(data)) {
                ...
                this.ticketsAll = data.map((u) => ({
                    ...
                    titulo: u.titulo,
                    descEstadoTicket:
                        u.estadoTicketNombre || 'No proporcionado',
                ...
                this.filteredTickets = this.ticketsAll.filter(
                    (ticket) => ticket.descEstadoTicket !== 'Cerrado'
                );
            } else {
                console.warn(
                    'No se recibieron tickets o los datos no son válidos'
                );
                this.ticketsAll = [];
    ...
    public filterTickets(filterType: string): void {
        if (filterType === 'closed') {
            ...
            this.filteredTickets = this.ticketsAll.filter(
                (ticket) => ticket.descEstadoTicket === 'Abierto'
            );
        }
    }
    ...
    public confirmEncuesta(rowData): void {
        console.log(rowData);
        this.confirmationService.confirm({
            key: 'ticket',
            header: 'Realizar Encuesta',
            message: `¿Estás seguro de Realizar la encuesta sobre el Ticket #${rowData.idTicket}?`,
            accept: () => {
                this.router.navigate(['/encuesta/encuesta', rowData.idTicket]);
            },
        });
    }
    ...
    async asignarAgente(
        ...
        this.departamentoAgentes = await lastValueFrom(
            this.getDepartamentoAgente()
        );
        ...
        this.ticketForm = this.fb.group({
            ...
            idAgente: [idAgente],
            titulo: [titulo],
            idEstadoTicket: [rowData.idEstadoTicket],
            departamentoAgente: [
                idDepartamentoA,
                this.validarRol.administrador() ||
                this.validarRol.agente() ||
        ...
        if (this.ticketForm && this.ticketForm.get('idDepartamentoA')) {
            const departamentoAgenteSeleccionado =
                this.departamentoAgentes.find(
                    (departamentoAgente) =>
