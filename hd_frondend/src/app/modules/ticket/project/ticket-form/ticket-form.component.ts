...
export class TicketFormComponent implements OnInit {
  ...
  private prioridadApi = `${environment.
    apiUrl}/controllers/prioridad.controller.php?op=todos`;
  ...
  getTicketDetalle(idTicket): Observable<any[]> {
    return this.http.get<any[]>(`${this.ticketDetalleApi}&idTicket=${idTicket}`);
  }
  ...
  public async loadData() {
    await this.loadEntities([
      { apiCall: this.getSla(), property: 'slas', mapFunction: (sla: any) => sla.nombre },
      { apiCall: this.getPrioridad(), property: 'prioridades', mapFunction: (prioridad: any) => prioridad.nombre },
      { apiCall: this.getDepartamentoAgente(), property: 'departamentoAgentes', mapFunction: (departamentoAgente: any) => departamentoAgente.nombre },
      { apiCall: this.getEstadoTicket(), property: 'estadoTickets', mapFunction: (estadoTicket: any) => estadoTicket.nombre }

    ]);
    this.activatedRoute.params.subscribe(async params => { this.idTicket = params['codigo']; })
    if (this.idTicket) {
      await this.loadEntities([
        { apiCall: this.getTicketDetalle(this.idTicket), property: 'mensajes', mapFunction: (ticketdetalle: any) => ticketdetalle.detalle }
      ]);
    }
    if (this.departamentoAgentes && this.departamentoAgentes.idDepartamentoA) {
      await this.loadEntities([
        { apiCall: this.getAgentes(this.departamentoAgentes.idDepartamentoA), property: 'agentes', mapFunction: (agente: any) => agente.agente }
      ]);
    }
    this.getTicket();
  }
  ...
  public fechaInicioAtencion(ticket: any) {
    this.confirmationService.confirm({
      ...
      accept: () => {
        const fechaActual = new Date().toLocaleString('sv-SE', { timeZone: 'America/Guayaquil' }).replace('T', ' ');
        this.ticketForm.patchValue({ fechaInicioAtencion: fechaActual });
  ...
  public confirmCrearActualizar(operation: string) {
    this.confirmationService.confirm({
      ...
      message: operation === 'op=insertar' ? '¿Está seguro de crear el ticket?' : '¿Está seguro de actualizar el ticket?',
      accept: () => {
  ...
  private formTicket(data: any): FormGroup {
    ...
    this.ticketForm = this.fb.group({
      ...
      titulo: [titulo,
        this.validarRol.administrador() || this.validarRol.usuario() ? Validators.required : null
      ],
    ...
    const prioridadSeleccionada = this.prioridades.find((prioridad) => prioridad.idPrioridad == idPrioridad)
    ...
    this.ticketForm.get('estadoTicket').valueChanges.subscribe((estadoTicket) => {
      const estadoTicketId = estadoTicket?.idEstadoTicket || estadoTicket;
      this.ticketForm.get('idEstadoTicket').setValue(estadoTicketId);
    });
    const estadoTicketSeleccionada = this.estadoTickets.find((estadoTicket) => estadoTicket.idEstadoTicket == idEstadoTicket)
      ...
      this.activatedRoute.params.subscribe(async params => {
        ...
        try {
          this.ticketDetalleForm = this.fb.group({
            ...
            idAgente: [this.validarRol.usuario()
              ? null
              : this.ticketForm.get('idAgente').value],
            idDepartamentoA: [this.ticketForm.get('idDepartamentoA').value],
            observacion: [],
            detalle: [, Validators.required],
            fechaDetalle: [],
            tipoDetalle: [this.tipoDetalle]
          });
          this.ticketDetalle = this.ticketDetalleForm.value as TicketDetalle;
  ...
  async enviarMensaje() {
    ...
    if (this.ticketDetalleForm.valid) {
      ...
      try {
        await lastValueFrom(this.postTicketDetalleData(formData, 'op=insertar'));
        this.showToast('success', 'Éxito', 'La operación se realizó correctamente.');
        setTimeout(() => {
        location.reload();
        }, 400);
